...
```
7. At the bottom of your screen there should be a terminal and console, the console speaks R, and the terminal spearks Linux. This means that the terminal can run github commands. Click on this button if you havent already
```{r echo=FALSE, fig.align='center', out.width='95%', purl=FALSE}

knitr::include_graphics("images/Terminal.png")
```
8. Type in the command `git remote upstream https://github.com/rstudio-education/stat545` into the terminal prompt. This will create an upstream remote repo that can be updated and merged with our modified version.  
**Warning** If you want to create new chapaters and also want to be able to continue to update files, then you cannot change chapter orders by modifying the names of the files. You must instead use `#` to denote a new chapter inside of the closest chapter, which bookdown will be able to automatically add with correct numbering.
8. Type in teh command `git config rerere.enable true` into the terminal prompt. This will make it so that you will not be reprompted with the same merge conflicts that you have already addressed

## Getting a free url

We are getting a url name from [freenom](https://www.freenom.com/). 

**Warning**This website just provides free domains for one year. The only catch is that these domains will only last for one year before expiring or costing money. Being free for one year is a gimmick to get you paying for a domain once you rely on people returning to that specific domain. You should probably find some other hosting if you plan on doing anything other than making a temp website. You should not circulate the website publically using freenom hosting. Luckily the proccess should be similar if you use other hosting.
