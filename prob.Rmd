...
What is the chance of not rolling a *2*?Since the chance of rolling a *2* is $1/6$ or
...
chance of getting two *1*s?If $16.\bar{6}$% of the time the first die is a *1*
...
P(A\text{ or } B) &=&P(\text{face card or }{\color{redcards}\diamondsuit})  \notag \\
 ...
 &=& 12/52 + 13/52 - 3/52 \notag \\
 &=& 22/52 = 11/26 \notag\end{aligned}$$
Equation ({reference-type="ref" reference="diamondFace"}) is an example of the .
...
In the data set with 3,921 emails, 367
...
probabilities. Table {reference-type="ref"
...
Equations ({reference-type="ref" reference="complementSumTo1"})
and ({reference-type="ref" reference="complementDisjointEquation"}) yields a very useful
...
$P(A^c)$ from Table {reference-type="ref" reference="diceProb"} on page . (c) Determine $P(A)$.[^17]

Consider again the probabilities from
Table {reference-type="ref" reference="diceProb"} and rolling two dice. Find the following
...
Example {reference-type="ref" reference="probOf2Ones"} provides a basic example of two independent
...
Example {reference-type="ref" reference="probOf2Ones"} (page ), where we calculated the probability
...
Examples {reference-type="ref"
reference="probOf2Ones"}
and {reference-type="ref"
...
used?If the student does not use
...
&& \quad= P(\text{\var{midterm} = \emph{other} and \underline{\color{black}\var{final} = A}}) + P(\text{\var{midterm} = A and \underline{\color{black}\var{final} = A}}) \\
