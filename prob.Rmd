...
P(\text{\var{inoculated} = no})=0.9608\end{aligned}$$ Among the
...
[^26]: \(a\)
    $P(\text{\var{parent} = not} | \text{\var{student} = not})$.
    (b) Equation ({reference-type="ref"
    reference="condProbEq"}) for conditional probability indicates we
    should first find
    $P(\text{\var{parents} = not and \var{student} = not})=0.32$
    and $P(\text{\var{student} = not})=0.51$. Then the ratio
