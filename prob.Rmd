...
A "die", the singular of dice, is a cube with six faces numbered *1*,
*2*, *3*, *4*, *5*, and *6*. What is the chance of getting *1* when
rolling a die?[\[probOf1\]]{#probOf1 label="probOf1"} If the die is
...
roll?[\[probOf1Or2\]]{#probOf1Or2 label="probOf1Or2"} *1* and *2*
...
What is the chance of getting either *1*, *2*, *3*, *4*, *5*, or *6* on
the next roll?[\[probOf123456\]]{#probOf123456 label="probOf123456"}
...
chance of getting two *1*s?[\[probOf2Ones\]]{#probOf2Ones
...
P(\text{rolling a \emph{1}})\end{aligned}$$ As we become more
...
P(\text{\emph{1} or \emph{2}}) = P(\text{\emph{1}})+P(\text{\emph{2}}) = 1/6 + 1/6 = 1/3\end{aligned}$$
...
&&P(\text{\emph{1} or \emph{2} or \emph{3} or \emph{4} or \emph{5} or \emph{6}}) \\
    &&\quad= P(\text{\emph{1}})+P(\text{\emph{2}})+P(\text{\emph{3}})+P(\text{\emph{4}})+P(\text{\emph{5}})+P(\text{\emph{6}}) \\
...
whether at least one big number appeared in an email (*big*). Of the
3,921 emails, 549 had no numbers, 2,827 had only one or more small
numbers, and 545 had at least one big number. (a) Are the outcomes
*none*, *small*, and *big* disjoint? (b) Determine the proportion of
emails with value *small* and *big* separately. (c) Use the Addition
Rule for disjoint outcomes to compute the probability a randomly
selected email from the data set has a number in it, small or big.[^3]
...
*6*$\}$. This set of all possible outcomes is called the ($S$) for
...
roll is *2* or *3*. Then the of $D$ represents all outcomes in our
...
\(a\) Compute $P(D^c) = P($rolling a *1*, *4*, *5*, or *6*$)$. (b) What
is $P(D) + P(D^c)$?[^15]
...
least *4*, i.e. $\{$*4*, *5*, \..., *12*$\}$. (c) The sum is no more
...
*1*s?[\[threeDice\]]{#threeDice label="threeDice"} The same logic
applies from
Example [\[probOf2Ones\]](#probOf2Ones){reference-type="ref"
...
P(white=\text{\small\emph{1} and } red=\text{\small\emph{1} and } blue=\text{\small\emph{1}})
    &= P(white=\text{\small\emph{1}})\times P(red=\text{\small\emph{1}})\times P(blue=\text{\small\emph{1}}) \\
...
$$\begin{aligned}
P(\text{\var{student} = \emph{uses} given \var{parents} = \emph{used}}) = \frac{125}{210} = 0.60\end{aligned}$$
...
$$\begin{aligned}
P(\text{\var{parents} = \emph{used} given \var{student} = \emph{not}}) = \frac{85}{226} = 0.376\end{aligned}$$
...
P(\text{\var{student} = \emph{uses}}) = \frac{219}{445} = 0.492\end{aligned}$$
A probability of outcomes for two or more variables or processes is
called a : $$\begin{aligned}
P(\text{\var{student} = \emph{uses} and \var{parents} = \emph{not}}) = \frac{94}{445} = 0.21\end{aligned}$$
...
&&P(\text{\underline{\color{black}\var{student} = \emph{uses}}}) \\
&& \quad =  P(\text{\var{parents} = \emph{used}, \underline{\color{black}\var{student} = \emph{uses}}}) + \\
&& \quad \quad \quad \quad P(\text{\var{parents} = \emph{not}, \underline{\color{black}\var{student} = \emph{uses}}}) \\
...
*used*. There are two parts to a conditional probability, and the . It
...
&& P(\text{\var{student} = \emph{uses} given \var{parents} = \emph{used}}) \notag \\
&& = P(\text{\var{student} = \emph{uses} } | \text{ \var{parents} = \emph{used}}) = \frac{125}{210} = 0.60
...
&& P(\text{\var{student} = \emph{uses} } | \text{ \var{parents} = \emph{used}}) \notag \\
&&\quad = \frac{\text{\# times \var{student} = \emph{uses} and \var{parents} = \emph{used}}}{\text{\# times \var{parents} = \emph{used}}} \label{ratioOfBothToRatioOfConditionalForParentsAndStudent} \\
...
P(\text{\var{student} = \emph{uses} } | \text{ \var{parents} = \emph{used}})
    &= \frac{\text{\# (\var{student} = \emph{uses} and \var{parents} = \emph{used})}}{\text{\# (\var{parents} = \emph{used})}} \notag \\
...
$$\begin{aligned}
P(\var{student} = \emph{uses}\ \text{and}\ \var{parents} = \emph{used})
    \quad\text{and}\quad
    P(\var{parents} = \emph{used}).\end{aligned}$$ The fraction of these
...
$$\begin{aligned}
P(\text{\var{parents} = \emph{used}}|\text{\var{student} = \emph{not}})
    + P(\text{\var{parents} = \emph{not}}|\text{\var{student} = \emph{not}})\end{aligned}$$
...
also affect whether that person was inoculated?[^32]
...
$$\begin{aligned}
P(\text{\var{result} = \emph{lived} and \var{inoculated} = \emph{no}})\end{aligned}$$
and we are given that $$\begin{aligned}
P(\text{\var{result} = \emph{lived} }|\text{ \var{inoculated} = \emph{no}})=0.8588 \\
P(\text{\var{inoculated} = \emph{no}})=0.9608\end{aligned}$$ Among the
96.08% of people who were not inoculated, 85.88% survived:
$$\begin{aligned}
P(\text{\var{result} = \emph{lived} and \var{inoculated} = \emph{no}}) = 0.8588\times 0.9608 = 0.8251\end{aligned}$$
...
die, $X$, is *1*? (b) What is the probability that both $X$ and $Y$ are
*1*? (c) Use the formula for conditional probability to compute $P(Y =$
*1* $| X =$ *1*$)$. (d) What is $P(Y=1)$? Is this different from the
answer from part (c)? Explain.[^36]
...
P(Y=\text{\emph{1}}|X=\text{\emph{1}})
    &=& \frac{P(Y=\text{\emph{1} and }X=\text{\emph{1}})}{P(X=\text{\emph{1}})} \\
    &=& \frac{P(Y=\text{\emph{1}})\times \color{oiGB}P(X=\text{\emph{1}})}{\color{oiGB}P(X=\text{\emph{1}})} \\
    &=& P(Y=\text{\emph{1}}) \\\end{aligned}$$
...
paycheck on red. What is wrong with his reasoning?[^37]
...
Rule: $$\begin{aligned}
&& P(\text{\var{inoculated} = \emph{yes} and \var{result} = \emph{lived}}) \\
    &&\quad = P(\text{\var{inoculated} = \emph{yes}})\times P(\text{\var{result} = \emph{lived}}|\text{\var{inoculated} = \emph{yes}}) \\
...
$P(\text{\var{midterm} = \emph{A}} | \text{\var{final} = \emph{A}})$. To
calculate this conditional probability, we need the following
probabilities: $$\begin{aligned}
P(\text{\var{midterm} = \emph{A} and \var{final} = \emph{A}}) \qquad\text{and}\qquad
P(\text{\var{final} = \emph{A}})\end{aligned}$$ However, this
...
probabilities: $$\begin{aligned}
&&P(\text{\var{midterm} = \emph{A} and \var{final} = \emph{A}}) = 0.0611 \\
&&P(\text{\underline{\color{black}\var{final} = \emph{A}}})  \\
&& \quad= P(\text{\var{midterm} = \emph{other} and \underline{\color{black}\var{final} = \emph{A}}}) + P(\text{\var{midterm} = \emph{A} and \underline{\color{black}\var{final} = \emph{A}}}) \\
...
$$\begin{aligned}
P(\text{\var{midterm} = \emph{A}} | \text{\var{final} = \emph{A}}) &=& \frac{P(\text{\var{midterm} = \emph{A} and \var{final} = \emph{A}})}{P(\text{\var{final} = \emph{A}})} \\
...
[^20]: (a) The abbreviations *RH* and *LH* are used for right-handed and
    ...
    P(\text{all five are \emph{RH}})
    &= P(\text{first = \emph{RH}, second = \emph{RH}, ..., fifth = \emph{RH}}) \\
    &= P(\text{first = \emph{RH}})\times P(\text{second = \emph{RH}})\times  \dots \times P(\text{fifth = \emph{RH}}) \\
...
[^26]: \(a\)
    $P(\text{\var{parent} = \emph{not}} | \text{\var{student} = \emph{not}})$.
    (b) Equation ([\[condProbEq\]](#condProbEq){reference-type="ref"
    reference="condProbEq"}) for conditional probability indicates we
    should first find
    $P(\text{\var{parents} = \emph{not} and \var{student} = \emph{not}})=0.32$
    and $P(\text{\var{student} = \emph{not}})=0.51$. Then the ratio
    represents the conditional probability: $0.32/0.51 = 0.63$.

[^27]: (a) This probability is
    $\frac{P(\text{\var{parents} = \emph{used} and \var{student} = \emph{not}})}{P(\text{\var{student} = \emph{not}})} = \frac{0.19}{0.51} = 0.37$.
...
[^30]: $P($ = *died* $|$ =
    *no*$) = \frac{P(\text{\var{result} = \emph{died} and \var{inoculated} = \emph{no}})}{P(\text{\var{inoculated} = \emph{no}})} = \frac{0.1356}{0.9608} = 0.1411$.

[^31]: $P($ = *died* $|$ =
    *yes*$) = \frac{P(\text{\var{result} = \emph{died} and \var{inoculated} = \emph{yes}})}{P(\text{\var{inoculated} = \emph{yes}})} = \frac{0.0010}{0.0392} = 0.0255$.
...
[^36]: Brief solutions: (a) $1/6$. (b) $1/36$.
    (c) $\frac{P(Y = \text{ \emph{1} and }X=\text{ \emph{1}})}{P(X=\text{ \emph{1}})} = \frac{1/36}{1/6} = 1/6$.
