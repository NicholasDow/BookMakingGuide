...
Type `git submodule add https://github.com/moderndive/ModernDive_book` to 
to add a folder full of the files that make the ModernDive book

Type `git submodule add https://github.com/rafalab/dsbook.git` to add that book.

if you want to make changes to the submodule then you must go into the submodule with `cd name/of/dir` then type `git checkout master`
...
git:
  submodules: false
```
so that travis does not clone the submodules every time it runs in the command line.

### Making Changes in the submodules
In order to track changes that are made in the submodule, cannot just add, commit, and push the change. You must first `cd nameofdir`, then commit the changes with: `git commit -am "commit message"` or its equivalent. After you have committed the change within the submodule, you can exit the submodule with `cd ..` and make the commit normally.
