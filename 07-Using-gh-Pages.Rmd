...
9)
This line clones the repository

`git clone https://github.com/moderndive/ModernDive_book.git`

This line basically enables regular expression in bash.

`shopt -s extglob`

This line copies all the files that are needed into the main directory

`cp -r ./ModernDive_book/!(docs|.DS_store|.git|*.Rproj|previous_versions|.travis.yml|_build.sh|_deploy.sh) .`


Delete the old folder if you want afterwards


go into _bookdown.yml and replace or add to repo with `repo: https://github.com/NicholasDow/GHpages-book`

push the changes and you should be able to see them on your website. You can then use `bookdown::clean_book(TRUE); bookdown::render_book('index.Rmd', 'bookdown::gitbook');` to get a version of the book's html files to be written into your docs folder. It is taking a while on my computer, but that might just be because my computer is slow. It takes a LONG time, so i am still waiting for output. If i was wrong use this command instead `bookdown::clean_book(TRUE); bookdown::render_book('index.Rmd', 'bookdown::html');`